...
if (!empty($_POST['email']) && !empty($_POST['password'])) {
    ...
    if ($isEmail) {
        // Login con email
        $records = $conn->prepare('SELECT id, email, password, esAdmin FROM users WHERE email = :loginInput');
        $records->bindParam(':loginInput', $loginInput);
        $records->execute();
    } else {
        // Login con nombre de usuario
        $username = '@' . $loginInput;
        $records = $conn->prepare('SELECT id, email, password, esAdmin FROM users WHERE username = :username');
